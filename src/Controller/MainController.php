...
class MainController extends AbstractController
{
    ...
    public function ListeCommandes(PersistenceManagerRegistry $doctrine, Request $request, PaginatorInterface $paginator): Response
    {
        ...
        if ($pays) {
            $queryBuilder->andWhere('p.nom = :pays')
        ...
        $paysList = $em->getRepository(Pays::class)->createQueryBuilder('p')
            ->select('p.nom')
            ->distinct()
            ->getQuery()
            ->getResult();
    ...
    public function BlockU($id, UserRepository $rep, PersistenceManagerRegistry $doctrine, SendMailService $mail): Response
    {
        ...
        // Envoi du mail
        $mail->sendMail(
            'gestion.users.secure@gmail.com',
            'Secure Print',
            $user->getEmail(),
            'Account Restriction',
            'block',
    ...
    public function listeTr(Request $request, PersistenceManagerRegistry $doctrine): Response
    {
        ...
        $paysRepository = $doctrine->getRepository(Pays::class);
        $allCountries = $paysRepository->findAll();

        // Create query to fetch filtered results
        $qb = $entityManager->createQueryBuilder();
        $qb->select('p')
            ->from(Pays::class, 'p')
            ->leftJoin('p.banques', 'b'); // Assuming 'banques' is the relation in Pays entity

        // Get filter value from request
        $selectedPaysId = $request->query->get('pays');

        // Apply filter only to displayed data
        if ($selectedPaysId) {
        ...
        return $this->render('main/tresoreries/listeTR.html.twig', [
            'paysWithBanques' => $paysWithBanques, // Filtered data
            'allCountries' => $allCountries, // Always contains all countries
            'image' => $image,
            'selectedPaysId' => $selectedPaysId, // Keep track of selected country
        ]);
    }
}
